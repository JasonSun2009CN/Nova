...
| Phase 3 | v0.3.0 | 完整航行系统 + 相对论视觉      | S22~S29                                                    | 🚧 S22 已提交未合并；S23~S29 已规划（顺序见第五节）                                   |
...
| 4    | **S25 可与 S23/S24 并行**                | 实时仪表盘是纯 UI + 引擎数据已具备（`cruisePlan` / `VoyageController` tick），不依赖渲染升级。                                                          |
...
- **S23 航行视图真实星表化（已定，ADR-0011）**：VoyageView 从 Canvas 2D 抽象星流（`VoyageStarFlow`）升级为 R3F 复用 `StarField`，相机立于 `currentStarId` 看向 `destStarId`。three ~967KB chunk 在航行启动时加载（与星图弹窗共享同一 chunk，弹窗已加载则无感）；深度分层复用现有 LOD，拖尾/光行差/蓝移在现有 ShaderMaterial 上加 uniform。
