...
- 交付新阶段时 **同步刷新三处**：本文档 + `HANDOFF.md` + `docs/ROADMAP.md`（现状已交付至 S23 航行视图真实星表化 + S25 仪表盘提前，规划至 S38）。
...
| S21    | Phase 2    | v0.2 | Phase 2 收尾：2.3 质量保障（60fps / 缩放平移流畅度 / 星表数据抽样校验）+ 验收标准走查，关闭 v0.2                                                                                                                                                           | ✅ 已提交           | `fbc3bae` / `68c80bd`                                     |
| S22    | Phase 3    | v0.3 | 统一飞行模型 + 引擎 γ 配置：合并 `travelDistance`/`cruisePlan` 为单一模型（d=β·γ·τ）+ `ENGINE_TIERS`（γ_max × 解锁）+ 可达性 API（所需 γ/最短专注/可达半径），退役 `RECOMMEND_MAX_GAMMA`（ADR-0012）+ 不可达阻止（SetupPanel 警告 + 禁用启动，S22 提前做） | 🚧 已提交           | `feature/S22-unified-flight-model`（未合并）              |
| S23    | Phase 3    | v0.3 | 航行视图真实星表化（修订，ADR-0011 + ADR-0014）：R3F 进 VoyageView，**主视角 = 目的地单星居中放大 + 暗星点阵背景**（光行差/拖尾撤销，StarField 加 opacity）；`VoyageStarFlow` 退役；**S25 仪表盘随本阶段提前交付**                                         | 🚧 已提交           | `feature/S23-voyage-real-stars`（未合并）                 |
| S24    | Phase 3    | v0.3 | γ 视觉 Doppler（前向蓝移）：目的地星 + 前方背景星随 γ 偏蓝（shader uniform 驱动，`dopplerFactor`/`blueShiftColor`），**只蓝不红**（红移已砍）                                                                                                              | 🚧 已提交           | `feature/S24-doppler-blueshift`（未合并）                 |
| S25    | Phase 3    | v0.3 | 实时仪表盘：主/客观双时间轴、当前速度 v/c、γ、已航行/剩余距离、ETA、引擎功率指示（已随 S23 提前交付，ADR-0014）                                                                                                                                            | 🚧 已交付（随 S23） | 同 S23                                                    |
| S26    | Phase 3    | v0.3 | 跃迁过渡动画：启动点火→星光加速→进入跃迁（3s）/ 到达减速→目标星真实变大→入轨（3s）/ 中断紧急刹车停中途                                                                                                                                                     | 🚧 已提交           | `feature/S26-warp-transition`（未合并）                   |
...
| Phase 3 | v0.3.0 | 完整航行系统 + 相对论视觉      | S22~S29                                                    | 🚧 S22/S23/S24 已合并 main；S26 已提交（用户手动合并中）；S27 引擎解锁已开发；S28/S29 已规划 |
