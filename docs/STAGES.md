...
- 交付新阶段时 **同步刷新三处**：本文档 + `HANDOFF.md` + `docs/ROADMAP.md`（现状已交付至 S27 引擎解锁，规划至 S38）。
...
| Phase 3 | v0.3.0 | 完整航行系统 + 相对论视觉      | S22~S29                                                    | ✅ S22/S23/S24/S25 已合并 main；🚧 S26 已提交（用户手动合并中）、S27 引擎解锁已提交未合并；S28/S29 已规划 |
...
| 8    | **Phase 4：S30 → S31 → S32 → S33 → S34** | S31 依赖 S30 统计 + 星图路径记录；S32 依赖历史 + 统计；S34 引入云端账户，建议最后且可推迟。                                                             |
