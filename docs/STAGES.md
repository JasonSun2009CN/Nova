...
| S27    | Phase 3    | v0.3 | 引擎等级解锁（γ 分级，按累计专注解锁）：`getUnlockedTier` / `getNextUnlock` 纯函数 + 设置页当前引擎状态/升级路径提示 + 信息卡最短专注/推荐/功率表按当前引擎 γ_max（`useHistoryStore.stats.totalFocusHours` 驱动）                                          | 🚧 已开发           | `feature/S27-engine-unlock`（未合并）        |
...
- **S27 引擎等级（已交付）** 与 S19/S20/S22 联动：`cruisePlan` / `recommendDestination` / `minFocusMinutes` / 引擎功率表的 γ 上限改为**当前引擎**（`getUnlockedTier(totalFocusHours)`，数据源 `useHistoryStore.stats.totalFocusHours`）；升级路径「再累计 X 解锁下一级」用 `getNextUnlock`。跃迁任务模式（按 γ_max 自动算时长）留待与任务模式 UI 一起评估。
