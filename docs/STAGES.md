...
- 交付新阶段时 **同步刷新三处**：本文档 + `HANDOFF.md` + `docs/ROADMAP.md`（现状已交付至 S26 跃迁过渡动画，规划至 S38）。
...
| S26    | Phase 3    | v0.3 | 跃迁过渡动画：启动点火→星光加速→进入跃迁（3s）/ 到达减速→目标星变大入轨（3s）/ 中断紧急刹车停中途；`VoyagePhase` 相位状态机（store 同步设置 + App 3s 定时推进）+ `warp-flow` 纯函数 + **近观星流** `NearFieldFlow`（launching/braking 挂载，只过渡期出现） | 🚧 已开发           | `feature/S26-warp-transition`（未合并）      |
...
| Phase 3 | v0.3.0 | 完整航行系统 + 相对论视觉      | S22~S29                                                    | 🚧 S22~S26 已交付（S22/S23/S24 已合并 main，S26 在 `feature/S26-warp-transition` 未合并；S25 仪表盘随 S23 提前）；S27~S29 已规划 |
