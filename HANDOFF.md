...
- **S37 体验打磨（分支 `feature/S37-polish`，含 Tauri 发布 CI）**：**无障碍**（`--color-deep-400` #7d7d7d→#858585 达 WCAG AA、`text-deep-500` 全部改 400、reduced-motion 下星图全览 autoRotate 禁用、14 处 px 字号转 rem）；**首次引导教程**（`OnboardingDialog` 4 张滑动卡片 + settings `hasCompletedOnboarding` + e2e dismissOnboarding helper 兼容 7 个 spec）；**i18n 中英**（`src/i18n/` zh/en 字典 + `useI18n` hook 读 settings `language` + header 中/EN 切换，核心 UI 已接入 t()，字典键/占位符一致性单测）；**首屏验证**（主包+state+db ≈ 70KB gzip，three/星表懒加载，已达标）。**Tauri 发布 CI**（`.github/workflows/release.yml`：tag 触发 5 平台构建——macOS aarch64+x86_64 / Windows x64 / Linux x64 / Linux aarch64(QEMU)，`softprops/action-gh-release` 发布；`tauri.conf.json` bundle targets→["app"]）。**CI run-failed 修复**（playwright expect 超时 15s + 测试超时 60s + starmap-perf 帧率测试 CI skip）。验证：**388 单测 / 42 文件**、`pnpm check` 4/4、build exit 0。
...
- **macOS 打包**：Tauri 2 骨架已配好（`src-tauri/`），**Rust 已装**（rustup 1.97.1 + aarch64/x86_64-apple-darwin targets），`pnpm tauri build` 可本地构建；CI 发布见 `release.yml`。
- **当前分支**：`feature/S37-polish`（含 S35 星表扩展 + 无障碍 + 首次引导 + i18n + Tauri 发布 CI，均未合并 main）。上一级链：S29~S33 已全部合并 main（S34 已移除，无需分享功能，用户决策）。基线：`pnpm check` 4/4、**388 单测**、42 test files，见下方 2.2。
...
**验证基线**：`pnpm check` 4/4（**388 单测**，42 test files）；e2e 3 浏览器全过（44 过 + 28 跳过，WebGL 仅 chromium）；`pnpm build` 主包 ~116KB（含 i18n 字典；three 967KB 懒加载进星图/航行 chunk）。
