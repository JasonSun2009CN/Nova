...
Phases 1–3 done (v0.1/v0.2/v0.3 closed): MVP timer (S11–S15), star-map navigation (S16–S21), full voyage system + relativistic visuals (S22–S29, incl. real-star voyage view, Doppler blueshift, warp transitions, engine unlock tiers, Web Audio white noise, runtime FPS governor). Phase 4 (v0.5) in progress: **S30 Captain's Log stats panel** (engine-layer aggregator `src/engine/stats/captains-log.ts` + header「日志」dialog: total focus/distance/explored stars/streak + heatmap + weekly/monthly charts + timeline) on branch `feature/S30-captains-log` (unmerged; S29 sits on `feature/S29-phase3-qa`). For anything architecture-level, **write a new ADR before code** — that is the project's rule.
